...
const conversPriceToNumber = (originalPrice)=>{
...
const convertPercentToNumber = (originalValue) => {
    const result = String(originalValue).replace('%','');
    const formattedNumber = Number(result);
    return isNaN(formattedNumber) ? 0 : formattedNumber;
}


// 필터 버튼 누름 -> min , max , discount 값을 받아옴 -> 값을 이용해 해당하는 물품을 추출 -> 다시 화면에 나타냄
export const setButtonEvent = (productList) => {
    filterButton.onclick = () =>{
        const maxPrice = conversPriceToNumber(maxPriceFilter.value) || MAX_PRICE; 
        const discountRate = convertPercentToNumber(discountFilter.value) || 0;
        const minPrice = conversPriceToNumber(minPriceFilter.value) || 0; 
...
export const setFilterEvent = () => {
    minPriceFilter.onfocus = (event) => {
        event.target.value = conversPriceToNumber(event.target.value);
    }
